...
class InitObjectUserNotAurhorized extends InitObject{
    ...
    public init(sizeCanvas: Size): GeometricObject[] {
        ...
        if (buttonCanvasElements.length > 0 && buttonCanvasElements[0] !== null) {
            ...
            const butLogIn: Rectangle | null = this.createObjectByElementId('button_canvas_login',
                new RectangleStyle({ font, strokeStyle: 'black', colorText: 'black', lineWidth: 2 }));

            if (butLogIn !== null) {
                butLogIn.addEvent('click', () => {
                    new ModalWindow('#modal_window_login').open();
                });
...
export default InitObjectUserNotAurhorized;